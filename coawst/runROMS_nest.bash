...
#exptMode="island"
#exptMode="tidesOnly"
exptMode="deadStillNesting"
#exptMode="nesting"
#exptMode="recode"
#exptMode="nestingDeadStill"
#exptMode="deadStill"
#exptMode="deadStill_MPDATA"
#exptMode="lj"
#exptMode="meso"
#exptMode="mesoNesting"
#exptMode="mesoTides_LMD"
#exptMode="mesoNoTides"
#exptMode="mesoNoTides_LMD"
#exptMode="meso_Eig"
#exptMode="tidesOnly_LMD"
...
#nameAppend="_UH_hasTides"
#nameAppend="_GLS"
#nameAppend="_CMEMS_GFS"
#nameAppend="_undefSCORRECTION"
#nameAppend="_newCode"
#nameAppend="_twoWay_LJ"
#nameAppend="_noNesting"
#nameAppend="_twoWayDebug"
#nameAppend="_M2"
#nameAppend="_twoWayShort"
#nameAppend="_oneWayShort"
#nameAppend="_undefSalinity"
#nameAppend="_oneWay"
#nameAppend="_WET_DRY"
nameAppend="_proto"
#nameAppend="_debug2WayNesting"
#nameAppend="_twoWay"
#nameAppend="_UHdata_UWic_UWbc_n50"
...
Ngrids=`grep 'Ngrids =' $runInputFile | rev | cut -d ' ' -f1 | rev`
if [[ $Ngrids -gt 1 ]];then
	echo "Nesting! Ngrids = " $Ngrids
else
	echo "no nesting"
fi
echo " "
...
gridFile=`grep GRDNAME $runInputFile | grep -v '!' | cut -d '\' -f1`
gridFileShort=`echo $gridFile | rev | cut -d '/' -f1 | rev `

nDum=`grep -n GRDNAME $runInputFile | grep -v '!' | cut -d ':' -f1`
nDum=`echo " $nDum + $Ngrids - 1" | bc` 

echo "main gridFile = $gridFile"
echo "gridFileShort = $gridFileShort"
echo ""
echo "The grid file(s) for this experiment is/are"
head -$nDum $runInputFile | tail -$Ngrids
echo "" 
...
if [ `grep -c ANA_INITIAL $runInputFile` == 1 ];then
	...
	echo "start from ini file(s)"
	echo ""
	if [[ $Ngrids -eq 1 ]];then
		ini_name=`grep ININAME $runInputFile | grep -v '!' | cut -d '=' -f3  `
	    echo "IC file name is  "$ini_name
	else
		nDum=`grep -n ININAME $runInputFile | grep -v '!' | cut -d ':' -f1`
		nDum=`echo " $nDum + $Ngrids - 1" | bc`
	
		echo "IC file names are"
		head -$nDum $runInputFile | tail -$Ngrids 
		ini_name=`head -$nDum $runInputFile | tail -$Ngrids | head -1  | cut -d '=' -f3 | cut -d '\' -f1 `
	fi 
	echo ""
...
#    echo "temp " $temp "  ini_name2 " $ini_name2
	...
	myYear=`grep ','$dstart',' ~/addl_Scripts/dayConverterCommas.txt | cut -d '-' -f1`
	myDay=`grep ','$dstart',' ~/addl_Scripts/dayConverterCommas.txt | tr -s ' ' | tr ' ' ',' | cut -d ',' -f4`


echo "myYear $myYear myDay $myDay"
...
if [[ $Ngrids -eq 1 ]];then
	...
	for tt in `seq 1 $Ngrids`
	do
        dtDum=`echo $dt | cut -d ' ' -f$tt`
		ntimesDum=`echo $ntimes | cut -d ' ' -f$tt`
		runDays=` echo " $ntimesDum * $dtDum / 86400 " | bc`
	    echo "run time on grid $tt is " $runDays " days"
	done
fi
echo ""
...
nhis=`grep "NHIS ==" $runInputFile |grep -v '! !' | cut -d "=" -f3 | cut -d '!' -f1`
nqck=`grep "NQCK ==" $runInputFile |grep -v '! !' | cut -d "=" -f3 | cut -d '!' -f1`
ndefhis=`grep "NDEFHIS ==" $runInputFile |grep -v '! !' | cut -d "=" -f3 | cut -d '!' -f1`
ndefqck=`grep "NDEFQCK ==" $runInputFile |grep -v '! !' | cut -d "=" -f3 | cut -d '!' -f1`
...
#echo " "
    ...
    do
        dtDum=`echo $dt | cut -d ' ' -f$tt`
        nhisDum=`echo $nhis | cut -d ' ' -f$tt`
        ndefDum=`echo $ndefhis | cut -d ' ' -f$tt`

	    hisSnapShot=` echo "scale=2; $dtDum * $nhisDum / 3600" | bc -l`
	    hisNewFile=` echo "scale=2; $dtDum * $ndefDum / 3600" | bc -l`
	    echo "hisSnapShot  = "$hisSnapShot  "hours    hisNewFile = "$hisNewFile" hours"
    done
    ...
    do
        dtDum=`echo $dt | cut -d ' ' -f$tt`
        nqckDum=`echo $nqck | cut -d ' ' -f$tt`
        ndefDum=`echo $ndefqck | cut -d ' ' -f$tt`

	    qckSnapShot=` echo "scale=2; $dtDum * $nqckDum / 3600" | bc -l`
	    qckNewFile=` echo "scale=2; $dtDum * $ndefDum / 3600" | bc -l`
	    echo "qckSnapShot  = "$qckSnapShot  "hours    qckNewFile = "$qckNewFile" hours"
...
# take a closer look at the size of the corner cell in case it's a telescoping grid
pm=`ncks -v pm -d xi_rho,0 -d eta_rho,0 $origGridName | tail -2 | head -1 | cut -d '=' -f2 | cut -d ' ' -f1`
pn=`ncks -v pn -d xi_rho,0 -d eta_rho,0 $origGridName | tail -2 | head -1 | cut -d '=' -f2 | cut -d ' ' -f1`
deltaXi=` echo " 1 / $pm " |bc `
deltaEta=`echo " 1 / $pn " |bc `
hypotenuse=`echo " sqrt ( ( $deltaXi * $deltaXi + $deltaEta * $deltaEta ) / 2 ) " |bc `

echo "On the other hand, the corner cell has dimensions  deltaXi = $deltaXi m  and deltaEta = $deltaEta m"
echo "  and the cell diagonal of this cell is $hypotenuse m"
...
if [[ $Ngrids -eq 1 ]];then
    TNU2=`grep "TNU2 =="    $runInputFile | grep -v ad_ | grep -v '^!' | cut -d "*" -f2 | cut -d "." -f1 `
    VISC2=`grep "VISC2 =="    $runInputFile | grep -v ad_ | grep -v '^!' | cut -d '=' -f3 | tr -d ' ' | cut -d '.' -f1 `
else
    VISC2=`grep "VISC2 =="    $runInputFile | grep -v ad_ | grep -v '^!' | cut -d "=" -f3 | cut -d " " -f2 | cut -d '*' -f2 | cut -d '.' -f1`
	TNU2=`grep "TNU2 =="    $runInputFile | grep -v ad_ | grep -v '^!' | cut -d "=" -f3 | cut -d " " -f2 | cut -d '*' -f2 | cut -d '.' -f1`
fi

echo "TNU2 = " $TNU2    "VISC2 = " $VISC2


# Calculate 'first guess' viscosity

viscCalc=`echo "  $hypotenuse  / 100. " | bc`
...
# check tnu2 and viscosity for a subgrid, if any
...
#	echo $nDum	
	nDum=`echo " $nDum + $Ngrids - 1" | bc`
#	echo $nDum
    subGridName=$gridName'/'`head -$nDum $runInputFile | tail -1 | cut -d '/' -f3-5`
	echo " "
    echo " "
	echo 'The SUBGRID file for this experiment is' $subGridName
	
	ncwa -O $subGridName gridAve.nc
	pm=`ncdump gridAve.nc |grep "pm =" | cut -d " " -f4`
	pn=`ncdump gridAve.nc |grep "pn =" | cut -d " " -f4`
...
#	echo "pm = $pm    pn = $pn"
	...
	# get tnu2 and visc2
    TNU2=`grep 'TNU2 ==' $runInputFile | grep -v ad_ | grep -v '^!' | cut -d '*' -f3 | cut -d '.' -f1`
	VISC2=`grep 'VISC2 ==' $runInputFile | grep -v ad_ | grep -v '^!' | cut -d 'd' -f2 | cut -d ' ' -f2-5 | cut -d '.' -f1 | tr -d ' ' `
...
lbcStart=`grep -n 'jgp LBC start' $runInputFile | cut -d ':' -f1`
lbcEnd=`  grep -n 'jgp LBC end'   $runInputFile | cut -d ':' -f1`
...
if [[ $Ngrids -eq 1 ]];then
	...
	nProc=0
#    for nn in `seq 1 $Ngrids`
	for nn in `seq 1 1`
	do
#		echo $nn
		dumx=`echo $tileX | cut -d " " -f$nn`
        dumy=`echo $tileY | cut -d " " -f$nn`
		nProc=`echo " $nProc + $dumx * $dumy " | bc`
#		echo $dumx $dumy $nProc
	done
fi


if [[ $nProc -gt 48 ]];then
	partition='t2standard'
else
    partition='t2small'
fi
...
echo " "
    ...
    echo '#SBATCH --partition='$partition                                                               >>$batName
...
#    echo 'fi'																							>>$batName
    ...
    echo "mpirun -np $nProc  $exeName ./Apps/ocean_$exptname.in > log"   								>>$batName 
